...
(function () {
  ...
  function sellForm() {
    ...
    pane.innerHTML = '<form>'
      ...
      + '<div class="f2"><div><label for="gp-s-cred">Licence or credential, if any</label><input id="gp-s-cred" placeholder="CRD, bar number, press card…"><p class="hint">Only a seller with one on file may mark work as advice.</p></div><div><label for="gp-s-ach">Your achpay.com address — how you are paid</label><input id="gp-s-ach" placeholder="the email achpay.com knows you by"><p class="hint">Gigapoo pays by ACH through achpay.com only. Card-paid gigs: Stripe&rsquo;s fee comes off your side.</p></div></div>'
  ...
  function mine(tok) {
    ...
    get(API + "/?" + q({ action: "me", token: tok })).then(function (d) {
      ...
      pane.querySelector("#gp-achpay").addEventListener("click", function (e) { e.preventDefault(); var a = prompt("Your achpay.com address — Gigapoo pays you there, and nowhere else:"); if (!a) return; get(API + "/?" + q({ action: "bank", token: tok, achpay: a })).then(function (r) { alert(r.ok ? r.note : (r.error || "did not save")); mine(tok); }); });
    }).catch(function () { pane.innerHTML = '<div class="warn">Could not reach the market.</div>'; });
  }
